...
// model family. We point at the INTERNATIONAL DashScope host (the alibabacloud.com console maps to
// the Singapore region), and the org configures a dedicated Wan key at Settings → Media (own-key,
// not the Qwen credential — see UNIVERSAL_AI_CREDENTIAL, which Wan is intentionally NOT part of).
//
// Image (Wan2.2 / Wanx t2i) and video (Wan2.x t2v + i2v) are both async task APIs: POST with
// `X-DashScope-Async: enable` returns a `task_id`, then poll GET /tasks/{id}. There is no completion
// webhook → video relies on the media-jobs-poll cron (like Runway/Veo/Qwen); image keeps the
// synchronous contract (§11.2) via bounded internal polling.
const BASE = 'https://dashscope-intl.aliyuncs.com/api/v1';

// DashScope splits a request into `input` (prompt + source refs) and `parameters` (everything else).
// The studio descriptor is flat, so route these names into `input`; all other native params (size,
// resolution, duration, n, seed, prompt_extend, watermark…) ride into `parameters`.
const INPUT_KEYS = new Set(['negative_prompt', 'img_url', 'audio_url']);
...
export class WanAdapter implements MediaProviderAdapter {
  constructor(private readonly _fetch: SafeFetchPort) {}
  readonly identifier = 'wan';
  readonly name = 'Wan';
  ...
  private _headers(options?: MediaCredentialOptions, async = false): Record<string, string> {
    ...
    if (!apiKey) throw new Error('Wan (Alibaba Model Studio) API key is required');
  ...
  // Wan2.2 / Wanx text-to-image is task-based; bounded internal polling keeps the synchronous image
  ...
  async generateImage(prompt: string, options?: MediaGenerateOptions): Promise<MediaGenerationResult> {
    const model = options?.model || 'wan2.2-t2i-flash';
  ...
  // Wan2.x text-to-video and image-to-video share one endpoint — the model id and the presence of
  // `input.img_url` (resolved server-side from the studio media picker) select the mode. No webhook →
  // completion comes from pollJob via the poll cron.
  ...
  async generateAudio(_prompt: string, _options?: MediaGenerateOptions): Promise<MediaJobSubmission> {
    throw new Error('Wan does not support audio generation');
  }

  async generateAvatar(_prompt: string, _options?: MediaGenerateOptions): Promise<MediaJobSubmission> {
    throw new Error('Wan does not support avatar generation');
  }
  ...
  async pollJob(jobId: string, options?: MediaCredentialOptions): Promise<MediaPollResult> {
    ...
    if (!resolveApiKey(options)) return { status: 'failed', error: 'Wan (Alibaba Model Studio) API key is required' };
    ...
    if (status === 'SUCCEEDED') {
      ...
      if (!artifactUrl) return { status: 'failed', error: 'Wan task succeeded without output' };
  ...
  // Cheap auth check (no generation cost) via the OpenAI-compatible models list on the same Model
  // Studio key — the studio's generateImage fallback would otherwise bill a render.
  async testConnection(options?: MediaCredentialOptions): Promise<{ ok: boolean; message: string }> {
    const apiKey = resolveApiKey(options);
    if (!apiKey) return { ok: false, message: 'Wan (Alibaba Model Studio) API key is required' };
    try {
      const res = await this._fetch('https://dashscope-intl.aliyuncs.com/compatible-mode/v1/models', {
        headers: { Authorization: `Bearer ${apiKey}` },
      });
      if (res.ok) return { ok: true, message: 'Connection successful' };
      return { ok: false, message: `Wan connection failed: ${await res.text()}` };
    } catch (err) {
...
const _meta = new WanAdapter(undefined as unknown as SafeFetchPort);

export const wanMediaModule: ProviderModule<any, any> = {
  ...
  create: (rt) => new WanAdapter(rt.fetch),
};
