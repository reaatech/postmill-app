...
import { SwitchyAdapter } from '../shortlink.adapter';
...
describe('SwitchyAdapter', () => {
  let adapter: SwitchyAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new SwitchyAdapter(safeFetch);
  });

  describe('metadata', () => {
    it('has identifier "switchy"', () => {
      expect(adapter.identifier).toBe('switchy');
    });

    it('has name "Switchy"', () => {
      expect(adapter.name).toBe('Switchy');
    });
    ...
    it('has defaultDomain "switchy.io"', () => {
      expect(adapter.defaultDomain).toBe('switchy.io');
    });
  ...
  describe('resolveDomain', () => {
    ...
    it('returns defaultDomain when no customDomain', () => {
      expect(adapter.resolveDomain(mockContext)).toBe('switchy.io');
    });
  });
  ...
  describe('validateCredentials', () => {
    it('returns ok: true on successful API response', async () => {
      ...
      expect(safeFetch).toHaveBeenCalledWith('https://api.switchy.io/v1/account', expect.objectContaining({
        method: 'GET',
    ...
    it('returns ok: false on non-200 response', async () => {
      ...
      expect(result.error).toContain('Switchy API error (401)');
  ...
  describe('createShortLink', () => {
    it('creates short link and returns shortUrl with providerLinkId', async () => {
      const responseBody = { short_url: 'https://switchy.io/abc123', id: 'abc123' };
      ...
      expect(result.shortUrl).toBe('https://switchy.io/abc123');
      expect(result.providerLinkId).toBe('abc123');
      expect(safeFetch).toHaveBeenCalledWith('https://api.switchy.io/v1/links', expect.objectContaining({
        method: 'POST',
      }));
    ...
    it('falls back to data.link and data.slug when short_url/id absent', async () => {
      const responseBody = { link: 'https://switchy.io/xyz', slug: 'xyz' };
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(mockResponse(200, responseBody));

      const result = await adapter.createShortLink(mockContext, 'https://example.com/long-url');
      expect(result.shortUrl).toBe('https://switchy.io/xyz');
    ...
    it('throws on API error response', async () => {
      ...
      await expect(adapter.createShortLink(mockContext, 'https://example.com'))
        .rejects.toThrow('Switchy create failed (400)');
