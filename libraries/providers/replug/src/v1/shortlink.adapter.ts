...
export class ReplugAdapter implements ShortLinkCapability {
  ...
  async validateCredentials(ctx: ShortLinkContext): Promise<{ ok: boolean; error?: string }> {
    try {
      const response = await this._fetch(`${this.apiBase}/v1/account/me`, {
        method: 'GET',
        headers: {
...
const _meta: ShortLinkCapability = new ReplugAdapter(undefined as unknown as SafeFetchPort);

export const replugShortlinkModule: ProviderModule<any, any> = {
  manifest: {
    domain: 'shortlink',
  ...
  create: (rt) => new ReplugAdapter(rt.fetch),
};
