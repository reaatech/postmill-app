...
import { ReplugAdapter } from '../shortlink.adapter';
...
describe('ReplugAdapter', () => {
  let adapter: ReplugAdapter;

  beforeEach(() => {
    vi.clearAllMocks();
    adapter = new ReplugAdapter(safeFetch);
  });

  describe('metadata', () => {
    it('has identifier "replug"', () => {
      expect(adapter.identifier).toBe('replug');
    });

    it('has name "Replug"', () => {
      expect(adapter.name).toBe('Replug');
    });
    ...
    it('has defaultDomain "replug.link"', () => {
      expect(adapter.defaultDomain).toBe('replug.link');
    });
  ...
  describe('resolveDomain', () => {
    ...
    it('returns defaultDomain when no customDomain', () => {
      expect(adapter.resolveDomain(mockContext)).toBe('replug.link');
    });
  });
  ...
  describe('validateCredentials', () => {
    it('returns ok: true on successful API response', async () => {
      ...
      expect(safeFetch).toHaveBeenCalledWith('https://api.replug.link/v1/account/me', expect.objectContaining({
        method: 'GET',
    ...
    it('returns ok: false on non-200 response', async () => {
      ...
      expect(result.error).toContain('Replug API error (401)');
  ...
  describe('createShortLink', () => {
    it('creates short link and returns shortUrl with providerLinkId', async () => {
      const responseBody = { short_url: 'https://replug.link/abc123', id: 'abc123' };
      ...
      expect(result.shortUrl).toBe('https://replug.link/abc123');
      expect(result.providerLinkId).toBe('abc123');
      expect(safeFetch).toHaveBeenCalledWith('https://api.replug.link/v1/links', expect.objectContaining({
        method: 'POST',
      }));
    ...
    it('falls back to data.link and data.slug when short_url/id absent', async () => {
      const responseBody = { link: 'https://replug.link/xyz', slug: 'xyz' };
      const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(mockResponse(200, responseBody));

      const result = await adapter.createShortLink(mockContext, 'https://example.com/long-url');
      expect(result.shortUrl).toBe('https://replug.link/xyz');
    ...
    it('throws on API error response', async () => {
      ...
      await expect(adapter.createShortLink(mockContext, 'https://example.com'))
        .rejects.toThrow('Replug create failed (400)');
