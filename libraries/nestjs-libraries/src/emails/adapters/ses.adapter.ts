...
export class SesAdapter implements EmailAdapter {
  ...
  private readonly _logger = new Logger(SesAdapter.name);
  ...
  async verifyWebhook(rawBody: Buffer, headers: Record<string, string | undefined>): Promise<boolean> {
    try {
      ...
      if (msgType === 'SubscriptionConfirmation') {
        ...
        try {
          const response = await safeFetch(subscribeUrl, { method: 'GET' });
          if (response.ok) {
  ...
  private async _verifySnsSignature(payload: any): Promise<boolean> {
    ...
    try {
      const certResponse = await safeFetch(certUrl, { method: 'GET' });
      if (!certResponse.ok) {
