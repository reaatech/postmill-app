...
@Injectable()
export class RebrandlyAdapter implements ShortLinkAdapter {
  ...
  async validateCredentials(ctx: ShortLinkContext): Promise<{ ok: boolean; error?: string }> {
    try {
      const response = await safeFetch(`${this.apiBase}/account`, {
        method: 'GET',
  ...
  async createShortLink(ctx: ShortLinkContext, originalUrl: string): Promise<{ shortUrl: string; providerLinkId?: string }> {
    ...
    const response = await safeFetch(`${this.apiBase}/links`, {
      method: 'POST',
      headers: this.headers(ctx),
  ...
  async expandShortLink(ctx: ShortLinkContext, shortUrl: string): Promise<string> {
    const response = await safeFetch(`${this.apiBase}/links?slashtag=${encodeURIComponent(shortUrl.replace(/https?:\/\//, '').split('/').pop() || '')}`, {
      method: 'GET',
      headers: this.headers(ctx),
    });
    if (!response.ok) {
  ...
  async linkStatistics(ctx: ShortLinkContext, links: string[]): Promise<ShortLinkStat[]> {
    ...
    for (const link of links) {
      try {
        ...
        const response = await safeFetch(`${this.apiBase}/links?slashtag=${encodeURIComponent(slashtag)}`, {
          method: 'GET',
          headers: this.headers(ctx),
        });
        if (response.ok) {
  ...
  async listLinks(ctx: ShortLinkContext, page: number): Promise<ShortLinkStat[]> {
    const response = await safeFetch(`${this.apiBase}/links?limit=50&skip=${(page - 1) * 50}`, {
      method: 'GET',
      headers: this.headers(ctx),
    });
    if (!response.ok) {
