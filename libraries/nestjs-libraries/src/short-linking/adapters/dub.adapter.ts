...
@Injectable()
export class DubAdapter implements ShortLinkAdapter {
  ...
  async validateCredentials(ctx: ShortLinkContext): Promise<{ ok: boolean; error?: string }> {
    try {
      const response = await safeFetch(`${this.apiBase(ctx)}/links?page=1&pageSize=1`, {
        method: 'GET',
  ...
  async createShortLink(ctx: ShortLinkContext, originalUrl: string): Promise<{ shortUrl: string; providerLinkId?: string }> {
    ...
    const response = await safeFetch(`${this.apiBase(ctx)}/links`, {
      method: 'POST',
      headers: this.headers(ctx),
  ...
  async expandShortLink(ctx: ShortLinkContext, shortUrl: string): Promise<string> {
    const response = await safeFetch(`${this.apiBase(ctx)}/links/info?url=${encodeURIComponent(shortUrl)}`, {
      method: 'GET',
      headers: this.headers(ctx),
    });
    if (!response.ok) {
  ...
  async linkStatistics(ctx: ShortLinkContext, links: string[]): Promise<ShortLinkStat[]> {
    ...
    for (const link of links) {
      try {
        const response = await safeFetch(`${this.apiBase(ctx)}/links/info?url=${encodeURIComponent(link)}`, {
          method: 'GET',
          headers: this.headers(ctx),
        });
        if (response.ok) {
  ...
  async listLinks(ctx: ShortLinkContext, page: number): Promise<ShortLinkStat[]> {
    const response = await safeFetch(`${this.apiBase(ctx)}/links?page=${page}&pageSize=50`, {
      method: 'GET',
      headers: this.headers(ctx),
    });
    if (!response.ok) {
