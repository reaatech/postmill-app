...
@Injectable()
export class TinyccAdapter implements ShortLinkAdapter {
  ...
  async validateCredentials(ctx: ShortLinkContext): Promise<{ ok: boolean; error?: string }> {
    try {
      ...
      const response = await safeFetch(`${this.apiBase}/shorten?${params.toString()}`, { method: 'GET' });
  ...
  async expandShortLink(ctx: ShortLinkContext, shortUrl: string): Promise<string> {
    ...
    const response = await safeFetch(`${this.apiBase}/expand?${params.toString()}`, { method: 'GET' });
    if (!response.ok) {
      const text = await response.text();
  ...
  async linkStatistics(ctx: ShortLinkContext, links: string[]): Promise<ShortLinkStat[]> {
    ...
    for (const link of links) {
      try {
        ...
        const response = await safeFetch(`${this.apiBase}/stats?${params.toString()}`, { method: 'GET' });
        if (response.ok) {
          const data = (await response.json()) as any;
