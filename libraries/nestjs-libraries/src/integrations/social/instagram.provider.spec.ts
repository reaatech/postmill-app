...
describe('Instagram - comment capabilities', () => {
  ...
  describe('fetchComments', () => {
    ...
    it('includes cursor in URL when provided', async () => {
      ...
      await provider.fetchComments('ig-1', 'tok', 'media-1', 'cursor-123', mockIntegration);
  ...
  describe('replyToComment', () => {
    ...
    it('sends POST to correct endpoint with message', async () => {
      ...
      await provider.replyToComment('ig-1', 'tok', 'media-1', 'parent-1', 'Nice!', mockIntegration);
  ...
  describe('likeComment', () => {
    ...
    it('uses POST for like and DELETE for unlike', async () => {
      ...
      await provider.likeComment('ig-1', 'tok', 'media-1', 'comment-1', true, mockIntegration);
      expect(fetchMock).toHaveBeenCalledWith(
      ...
      fetchMock.mockClear();
      await provider.likeComment('ig-1', 'tok', 'media-1', 'comment-1', false, mockIntegration);
      expect(fetchMock).toHaveBeenCalledWith(
        expect.stringContaining('comment-1/likes'),
